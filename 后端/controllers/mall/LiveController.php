...
use app\forms\mall\live\LivePendantEditForm;
use app\forms\mall\live\LivePendantForm;
...
class LiveController extends MallController
{
    ...
    public function actionPendantSetting()
    {
        if (\Yii::$app->request->isAjax) {
            if (\Yii::$app->request->isPost) {
                $form = new LivePendantEditForm();
                $form->attributes = \Yii::$app->request->post();
                return $this->asJson($form->save());
            }

            $form = new LivePendantForm();
            return $this->asJson($form->getSetting());
        }

        return $this->render('pendant-setting');
    }

    public function actionPendantGoods()
    {
        if (\Yii::$app->request->isAjax) {
            $form = new GoodsForm();
            $form->attributes = \Yii::$app->request->get();
        ...
        return $this->render('pendant-goods');
    }
}
