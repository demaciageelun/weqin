...
use app\plugins\scratch\models\ScratchBout;
...
class ScratchForm extends Model
{
    ...
    private function validated(ScratchSetting $setting)
    {
        ...
        $this->freeModel = ScratchBout::getBout();
    ...
    public function receive()
    {
        ...
        try {
            ...
            if ($this->freeModel && $this->freeModel->bout) {
                $this->freeModel->bout--;
                $scratchLog->bout_id = $this->freeModel->id;
                $this->freeModel->save();
            } else if ($setting->deplete_integral_num > 0) {
