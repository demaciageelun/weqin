...
class GiftListForm extends Model
{
    ...
    public function getSendList()
    {
        ...
        try {
            ...
            if ($this->platform) {
                $model = PlatformConfig::getInstance()->searchPlatform($model, $this->platform, 'gl');
            }
            ...
            foreach ($list as &$item) {
                ...
                foreach ($item['user']['userPlatform'] as &$value) {
                    $value = (object)$value;
                }
                unset($value);
                $item['user']['userInfo'] = (object)$item['user']['userInfo'];
                $userPlatformDetail = PlatformConfig::getInstance()->getPlatformDetail((object)$item['user']);
                $item['user'] = [
