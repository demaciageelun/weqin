...
class LotteryJob extends BaseJob implements JobInterface
{
    public $model;
        ...
        foreach ($user as $item) {
            try {
                $this->sendTemplate([
                    ...
                    'goodsName' => $name,
                    'result' => '已中奖',
                    'remark' => '尽快来领取属于你的专属礼品！',
                    'user' => $item,
                    'page' => 'plugins/lottery/prize/prize?type=2',
                ]);
        ...
        foreach ($user as $item) {
            try {
                $this->sendTemplate([
                    ...
                    'remark' => '很抱歉，本次尚未中奖！',
                    'user' => $item,
                    'page' => 'plugins/lottery/index/index',
                ]);
            } catch (\Exception $exception) {
                \Yii::warning($exception->getMessage());
            }
    ...
    public function otherRefundMsg($list, $lottery, $remark)
    {
        foreach ($list as $v) {
            try {
                $this->sendTemplate([
                    'activityName' => '幸运抽奖',
                    'goodsName' => $lottery->goods->goodsWarehouse->name,
                    'result' => '未中奖',
                    'remark' => $remark,
                    'user' => $v->user,
                    'page' => 'plugins/lottery/index/index',
    ...
    {
        try {
            TemplateList::getInstance()->getTemplateClass(LotteryInfo::TPL_NAME)->send([
