...
class OrderSubmitForm extends \app\forms\api\order\OrderSubmitForm
{
    ...
    public function getAddressEnable($address, &$mchItem)
    {
        $result = parent::getAddressEnable($address, $mchItem);
        foreach ($mchItem['goods_list'] as $goodsItem) {
            if (isset($goodsItem['address_disabled'])) {
                $addressEnable[$goodsItem['id']] = $goodsItem['address_disabled'];
            }
        }
        if (!empty($addressEnable)) {
            foreach ($mchItem['composition_list'] as &$composition) {
                foreach ($composition['goods_list'] as &$goodsItem) {
                    if (isset($addressEnable[$goodsItem['id']])) {
                        $goodsItem['address_disabled'] = $addressEnable[$goodsItem['id']];
                    }
                }
                unset($goodsItem);
            }
