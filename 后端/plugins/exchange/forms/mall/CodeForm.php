        ...
        try {
            ...
            $list = array_map(function ($item) use ($library, $qrcode) {
                ...
                if (in_array($item['status'], [2, 3])) {
                    ...
                    $item['platform'] = PlatformConfig::getInstance()->getPlatform($u);
                    $item['platform_text'] = PlatformConfig::getInstance()->getPlatformText($u);
                    $item['platform_icon'] = PlatformConfig::getInstance()->getPlatformIcon($u);
                } else {
                    $item['platform'] = '';
                    $item['platform_text'] = '';
                    $item['platform_icon'] = '';
