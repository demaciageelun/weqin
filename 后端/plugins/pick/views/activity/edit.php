        ...
        <el-dialog title="编辑库存" :visible.sync="stockVisible" :before-close="stockClose">
            <div class="set-inventory">
                <div class="input" flex="main:center cross:center">
                    ...
                    <div>
                        <el-input size="small" v-model="stock" oninput="this.value = this.value.replace(/[^0-9]/g, '');" :min="0" type="number" @change="setStock">
            ...
            <el-table
                ...
                <el-table-column
                        ...
                        label="库存">
                    <template slot-scope="scope">
                        <el-input v-model="scope.row.stock" oninput="this.value = this.value.replace(/[^0-9]/g, '');" :min="0" type="number"></el-input>
                    </template>
                </el-table-column>
            </el-table>
                                                ...
                                                <template slot-scope="scope">
                                                    ...
                                                    <div style="display: flex;align-items: center" v-else>
                                                        <el-input style="min-width: 70px" type="number" size="mini" class="change"
                                                                  v-model="scope.row.sort" :max="9999999"
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            changeSortSubmit(row) {
                if (!row.sort) {
                ...
                }else if (row.sort > 9999999) {
                    row.sort = this.sort;
                    this.$message.warning('排序值过长');
                }else if (row.sort < 0) {
                    ...
                    this.editSaveGoods([data], 'edit', 'sort');
                }
            },
            ...
            async editSaveGoods(pick, type, item) {
                const e = await request({
                    params: {
                ...
                if (e.data.code === 0) {
                    ...
                    if(item == 'sort') {
                        for(let item of this.activityGoods) {
                            if(item.goods_warehouse_id == pick[0].goods_warehouse_id) {
                                item.sort = this.sort;
                            }
                        }
                    }
            ...
            sureStock() {
                ...
                for (let i = 0; i < this.stockList.length; i++) {
                    ...
                    if(this.stockList[i].stock > 0 || this.stockList[i].stock === 0) {
                        stock += Number(this.stockList[i].stock);
                    }else {
                        let attrText = '';
                        for(let item of this.stockList[i].attr_list) {
                            attrText += ' ' + item.attr_group_name + ':' + item.attr_name
                        }
                        this.$message({
                            type: 'warning',
                            message: '请填写' + attrText + ' 库存'
                        });
                        return false;
                    }
                }
                if (stock > 9999999) {
                    this.$message.error('商品总库存不能超过9999999');
                    return '';
                }
