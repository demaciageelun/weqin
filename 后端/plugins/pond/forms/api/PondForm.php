...
use app\plugins\pond\models\PondBout;
...
class PondForm extends Model
{
    ...
    private function validated(PondSetting $setting)
    {
        ...
        $this->freeModel = PondBout::getBout();
        if (empty($this->freeModel) || empty($this->freeModel->bout)) {
            //积分检测
    ...
    public function index()
    {
        try {
            ...
            return [
                ...
                'msg' => '',
                'data' => [
                    'free_oppty' => $this->freeModel ? intval($this->freeModel->bout) : 0,
    ...
    public function lottery()
    {
        try {
            ...
            //记录
            $this->saveLog($id, $setting->deplete_integral_num, $err, $use_oppty);
            $oppty = $setting->oppty - $this->log - $use_oppty;
            if ($this->freeModel && intval($this->freeModel->bout)) {
                $msg = '';
            } else if ($oppty <= 0) {
                $msg = '抽奖机会不足';
            } elseif (\Yii::$app->currency->setUser(\Yii::$app->user->identity)->integral->select() < $setting->deplete_integral_num) {
                $msg = '积分不足';
            } else {
                $msg = '';
            }
    ...
    //记录
    private function saveLog($id, $deplete_integral_num, $err, &$use_oppty)
    {
        $use_oppty = 1;
        /* @var Pond $pond */
        ...
        //FREE
        if ($this->freeModel && $this->freeModel->bout) {
            $this->freeModel->bout--;
            $use_oppty = 0;
            $pondLog->bout_id = $this->freeModel->id;
            $this->freeModel->save();
        } else if ($deplete_integral_num > 0) {
