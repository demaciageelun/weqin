...
class GoodsForm extends ApiModel
{
    ...
    public function search()
    {
        ...
        try {
            ...
            $commonGoodsDetail->setIsLimit($setting['is_territorial_limitation'] == 1 && $commonGoodsDetail->goods->goodsWarehouse->type != 'ecard');
            $commonGoodsDetail->setIsLimitBuy(true);
            $sales = $bargainGoods->participant + $bargainGoods->goods->virtual_sales; // 正在进行砍价人数
            $newGoods = [
                'name' => $goodsWarehouse->name,
                'subtitle' => $goodsWarehouse->subtitle,
                'keyword' => $goodsWarehouse->keyword,
                ...
                'param_name' => $bargainGoods->goods->param_name,
                'param_content' => \yii\helpers\BaseJson::decode($bargainGoods->goods->param_content),
