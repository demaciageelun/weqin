...
class GoodsForm extends Model
{
    ...
    public function search()
    {
        ...
        $limitData = \yii\helpers\BaseJson::decode($bargain->limit_data);

        $limitData = [
            'limit_status' => $limitData['limit_status'] ?? 0,
            'limit_i_status' => $limitData['limit_i_status'] ?? '',
            'limit_i_type' => $limitData['limit_i_type'] ?? 'day',
            'limit_i_num' => $limitData['limit_i_num'] ?? '',
            'limit_h_status' => $limitData['limit_h_status'] ?? '',
            'limit_h_type' => $limitData['limit_h_type'] ?? 'day',
            'limit_h_num' => $limitData['limit_h_num'] ?? '',
        ];
        $list = [
        ...
        $list = array_merge($list, $limitData);
        $detail['plugin'] = $list;
