...
class CommonBargainGoods extends Model
{
    ...
    public static function calcTime($type, &$text, &$sub_text)
    {
        switch ($type) {
            case 'day':
                $text = sprintf('今日%s次数已用完', $text);
                $sub_text = '明天再来吧';
                return date('Y-m-d 00:00:00');
            case 'week':
                $text = sprintf('本周%s次数已用完', $text);
                $sub_text = '下周再来吧';
                $interval_day = (date('w') == 0 ? 6 : (date('w') - 1));
                return (new \DateTime('now'))->sub(new \DateInterval('P' . $interval_day . 'D'))->format('Y-m-d 00:00:00');
            case 'month':
                $text = sprintf('本月%s次数已用完', $text);
                $sub_text = '下个月再来吧';
                return date('Y-m-1 00:00:00');
            case 'all':
                $text = sprintf('%s次数已用完', $text);
                $sub_text = '';
                return false;
        }
    }
    ...
    public function getApiGoodsList($page, $limit = 10)
    {
        ...
        $bargainGoodsList = BargainGoods::find()->alias('bg')->with(['goods'])
