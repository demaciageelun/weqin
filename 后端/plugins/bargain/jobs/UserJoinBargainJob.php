...
class UserJoinBargainJob extends BaseJob implements JobInterface
{
    ...
    public function execute($queue)
    {
        ...
        try {
            ...
            $limit_data = \yii\helpers\BaseJson::decode($this->bargainOrder->bargainGoods->limit_data);
            if (!empty($limit_data)
                && $limit_data['limit_status']
                && $limit_data['limit_h_status'] === 'cycle'
                && $this->bargainOrder->user_id != $this->user->id
            ) {
                //去除参与
                $childQuery = BargainOrder::find()->select('id')->where([
                    'mall_id' => $this->mall->id,
                    'bargain_goods_id' => $this->bargainOrder->bargainGoods->id,
                    'is_delete' => 0,
