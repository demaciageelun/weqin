...
class WechatWxmpprograms extends \app\models\ModelActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['mall_id', 'verify_type_info', 'is_show', 'idc', 'authorizer_expires', 'is_delete', 'version'], 'integer'],
            [['func_info'], 'string'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'is_show' => '是否显示，0显示，1隐藏',
            'user_name' => '原始ID',
            'qrcode_url' => '二维码图片的URL',
            'business_info' => 'json格式。用以了解以下功能的开通状况（0代表未开通，1代表已开通）： open_store:是否开通微信门店功能 open_scan:是否开通微信扫商品功能 open_pay:是否开通微信支付功能 open_card:是否开通微信卡券功能 open_shake:是否开通微信摇一摇功能',
            'idc' => 'idc',
            'principal_name' => '公众号的主体名称',
            'signature' => '帐号介绍',
            'miniprograminfo' => 'json格式。判断是否为小程序类型授权，包含network小程序已设置的各个服务器域名',
            'func_info' => 'json格式。权限集列表，ID为17到19时分别代表： 17.帐号管理权限 18.开发管理权限 19.客服消息管理权限 请注意： 1）该字段的返回不会考虑小程序是否具备该权限集的权限（因为可能部分具备）。',
            'authorizer_appid' => '公众号appid',
            'authorizer_access_token' => '授权方接口调用凭据（在授权的公众号或小程序具备API权限时，才有此返回值），也简称为令牌',
            'authorizer_expires' => 'refresh有效期',
            'authorizer_refresh_token' => '接口调用凭据刷新令牌',
            'created_at' => '授权时间',
