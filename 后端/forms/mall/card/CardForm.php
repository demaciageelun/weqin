...
class CardForm extends Model
{
    ...
    {
        return [
            [['id', 'user_card_id', 'is_expired', 'is_vip', 'vip_level'], 'integer'],
    ...
    public function send()
    {
        ...
        try {
            ...
            if ($this->is_vip) {
                if (empty($vip_levle = $this->vip_level)) {
                ...
                $query->keyword($this->vip_level, ['d.member_level' => $this->vip_level]);
                $this->user_id_list = $query->select(['u.id']);
            }
            if(empty($this->user_id_list)){
                throw new \Exception('发放用户不能为空');
            }
