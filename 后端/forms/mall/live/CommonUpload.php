...
class CommonUpload extends Model
{
     ...
     * @return string              [图片ID]
     */
    public function uploadImage($accessToken, $picUrl, $size = 10, $pxSize = array(), $autoExactPx = array(), $imageName = '')
    {
        ...
        try {
            ...
            if (!empty($pxSize)) {
                if ($width > $pxSize[0] || $height > $pxSize[1]) {
                    throw new \Exception($imageName . ",图片宽高最大限制为" . $pxSize[0] . '*' . $pxSize[1]);
                }
            }
            ...
            if (is_numeric($size)) {
                if ($imageInfo->size >= $size * 1024 * 1024) {
                    throw new \Exception($imageName . ",请检查图片大小,图片大小不能超过" . $size . 'MB');
                }
            } else if (strpos($size, 'KB') !== false) {
                $size = str_replace('KB', '', $size);
                if ($imageInfo->size >= $size * 1024) {
                    throw new \Exception($imageName . ",请检查图片大小,图片大小不能超过" . $size . 'KB');
                }
            } else {
                throw new \Exception($size . '参数异常'); 
            }
