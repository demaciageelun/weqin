...
class CommonAdminUser
{
    ...
    public static function createAdminUser($data)
    {
        ...
        if (isset($data['subjoin_permissions']) && is_array($data['subjoin_permissions'])) {
            $adminInfo->subjoin_permissions = \Yii::$app->serializer->encode($data['subjoin_permissions']);
        }
        $adminInfo->expired_at = $data['expired_at'];
        $adminInfo->user_group_id = isset($data['user_group_id']) ? $data['user_group_id'] : 0;
        $res = $adminInfo->save();
        if (!$res) {
