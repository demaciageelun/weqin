...
class CommonGoodsList extends Model
{
        ...
        if ($sign) {
            try {
                ...
                if (is_array($sign) && false !== $k = array_search('pintuan', $sign)) {
                    unset($sign[$k]);
                    sort($sign);
                    $ptIds = PintuanGoods::find()->select('goods_id')->where(['is_delete' => 0])->all();
                    $this->query->andWhere([
                        'OR',
                        ['g.sign' => 'pintuan', 'g.id' => $ptIds],
                        ['g.sign' => $sign]
                    ]);
                } else if (is_string($sign) && $sign === 'pintuan') {
                    $ptIds = PintuanGoods::find()->select('goods_id')->where(['is_delete' => 0])->all();
                    $this->query->andWhere(['g.sign' => 'pintuan', 'g.id' => $ptIds]);
                } else {
    ...
    private function keywordWhere()
    {
        $goodsIds = GoodsWarehouse::find()->andWhere(['is_delete' => 0])
            ->keyword($this->keyword !== '', ['or', ['like', 'name', $this->keyword], ['like', 'keyword', $this->keyword]])->select('id');
        $this->query->andWhere([
            'or',
