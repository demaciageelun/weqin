...
class CityServiceMapForm extends Model
{
    ...
    public function cityMap()
    {
        ...
        try {
            ...
            // TODO 同城速送
            if ($cityService->distribution_corporation == 5) {
                $url = \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl;
                $corporation = [
                    'name' => '同城速送',
                    'icon' => $url . '/statics/img/mall/city_service/mk.png',
                    'value' => 5,
                    'delivery_id' => 'MK',
                ];
            }
    ...
    private function getStatusAddress($deliveryId, $cityInfo, $status)
    {
        ...
        if (isset($cityInfo[$status])) {
            if ($deliveryId == 'SFTC') {
                ...
                // TODO
            } elseif ($deliveryId == 'MK') {
                $data['manLongitude'] = $cityInfo[$status]['data']['order_rider']['longitude'];
                $data['manLatitude'] = $cityInfo[$status]['data']['order_rider']['latitude'];
            }
        }
